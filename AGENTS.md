...
- Intelligence layer (`llm/compose.py`, `core/intelligence.py`): LLM headspace fusion, moment-fit check, multi-step digest. Bandit + hard gates stay deterministic. Cluster digest uses Exa web grounding (`GROUNDING_PROVIDER=exa`, `DIGEST_USE_WEB_GROUNDING`, default on) — Exa retrieves, Gemini synthesizes; fails soft (ungrounded) without `EXA_API_KEY`. Batch `bookmarks research` runs `link_fetch` (HTTP preview) before Exa-grounded Gemini research; results persist on bookmark docs for digest link cards.
